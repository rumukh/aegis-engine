...
**Every command and every output block in this guide was executed against this repository.**
Where something does not work, it says so instead of showing what it ought to do. The list of
known rough edges is [§9](#9-known-rough-edges) — read it before you trust a command.
...
This is not hypothetical: it is what the starter test from `aegis scaffold game` does today. That
test does contain a real-looking assertion — it is not empty — but the fact it checks is
established by scene instantiation and cannot be changed by anything that happens afterwards:
...
that the run never touches. Replace the scaffolded assertion before you build on it.
...
which makes it look like a working starting point, and its single assertion cannot fail for any
reason connected to the run — see the measurement in
[§6.5](#65-mutation-check-your-own-test). Do not build on it; replace its `expect(result)`
callback wholesale.
