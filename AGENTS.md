...
| 7 | **The `packages/render-three` browser tests are the suite''s only host-sensitive files, and vitest must be told so — but that is one cause of two, and the second is not what three rounds of repair assumed.** They pass on `ubuntu-latest` and on a development box; on `windows-latest` they failed thirteen pushes to `main` in a row, always in `browser-playability.test.ts`, never with a statement about the product. Run 30377421271: ubuntu ran 72 files / 1020 tests green with that file at 47.7s; windows ran the same corpus with that file at **607.3s and 7 failures**. The file''s CPU sampler had already named a cause: _"this process held a CPU only 0% of the window — it was NOT SCHEDULED."_ So `vitest.config.ts` now runs those two files as their own project, in `sequence.groupOrder: 1`, under `poolOptions.forks.singleFork`. That fixed `browser-diagnostics.test.ts` (33 of 33) and two of the seven — **and run 30380984122 then showed the same 0% verdict with the machine entirely to itself**, which is the useful part: _"oversubscribed by something outside this process"_ was true and named nothing that was still running. Two rounds were then spent on the reading that the process was blocked on I/O, and **both were wrong, refuted by the instruments added alongside them.** Free memory was 5.3GB of 8.0GB, so nothing was paging. And the blank-page control — added to establish a floor — settled it by accident: **windows measured 64fps / 15.9ms median gap against ubuntu''s 60fps / 16.7ms.** The failing host is not slower. It is _indistinguishable_ while idle, and diverges only once a page rasterises: from the same commit in the same run, the fps game ticked 65.0/s on ubuntu and **7.0/s** on windows. A 9x gap that appears only under GL load is not a disk story, and the Defender exclusion and Chrome startup flags that story bought are kept on the weaker claim they can be defended on — they remove real work and cannot move a number this file reports — rather than as the repair. The measurement that discriminates is which GL implementation actually answered, which nothing here had ever taken; the file now reports `renderer:` unconditionally, so it prints on the green leg too, because a suspect string is worth nothing without the passing side to compare it to. | Adding any new browser-launching test to `BROWSER_TEST_FILES` in `vitest.config.ts`. `test/browser-project-membership.test.ts` fails if you forget, because that regression is invisible everywhere except the leg you are not watching. Note `fileParallelism: false` looks like the setting for this and is not — vitest lists it in `NonProjectOptions`, so inside a project it is ignored silently. And before believing any verdict about _why_ a host is slow, check whether you have measured the host at all or only the failing arm: `--use-angle=swiftshader` is a request, not a guarantee, and three rounds of repair here were aimed at a mechanism the green leg would have ruled out in one line. |
