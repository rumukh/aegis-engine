...
| 7 | **The `packages/render-three` browser tests are the suite's only host-sensitive files, and vitest must be told so — but that was one of two causes, not the cause.** They pass on `ubuntu-latest` and on a development box; on `windows-latest` they failed thirteen pushes to `main` in a row, always in `browser-playability.test.ts`, never with a statement about the product. Run 30377421271: ubuntu ran 72 files / 1020 tests green with that file at 47.7s; windows ran the same corpus with that file at **607.3s and 7 failures**. The file's CPU sampler had already named a cause: _"this process held a CPU only 0% of the window — it was NOT SCHEDULED."_ So `vitest.config.ts` now runs those two files as their own project, in `sequence.groupOrder: 1`, under `poolOptions.forks.singleFork`. That fixed `browser-diagnostics.test.ts` (33 of 33) and two of the seven — **and run 30380984122 then showed the same 0% verdict with the machine entirely to itself**, which is the useful part: _"oversubscribed by something outside this process"_ was true and named nothing that was still running. What the transport actually showed was neither a climb nor a cliff but a stutter — `8/104/69/102/27/61/118/2/`**`28904`**`/165/12/348ms` — a process stopped dead for 25–40s at a stretch and then resumed healthy. Not scheduled _and_ not burning CPU is a process waiting on the OS, so the remaining work is aimed there: Chrome no longer does startup network fetches or keeps disk caches, and the Windows leg excludes its own workspace, TEMP, node and the browser from Defender. Whether that was enough is a claim to re-measure, not to read here. | Adding any new browser-launching test to `BROWSER_TEST_FILES` in `vitest.config.ts`. `test/browser-project-membership.test.ts` fails if you forget, because that regression is invisible everywhere except the leg you are not watching. Note `fileParallelism: false` looks like the setting for this and is not — vitest lists it in `NonProjectOptions`, so inside a project it is ignored silently. And when a wedge message says the box is oversubscribed, read the `Host:` clause beside it before believing it is CPU: `describeHost()` exists because that fork — contention or paging, which have opposite repairs — cost a CI round trip to even pose. |
