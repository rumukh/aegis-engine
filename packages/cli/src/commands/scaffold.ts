...
const USAGE = [
  ...
  '                    "game" writes aegis.json + a scene (with its tilemap embedded under',
  '                    resources) + an input script + a starter GameTest into <name>/, ready',
  '                    for: aegis validate / run / test.',
...
function testDoc(name: string, mode: GameMode, sceneRel: string): string {
  ...
  return [
    ...
    `// The fallback is taken only when Node says it cannot find THIS package. ERR_MODULE_NOT_FOUND`,
    `// is also what a present-but-broken package raises for its own missing internals, and`,
    `// swallowing that would run the CLI's copy of the mode instead of yours — green, against`,
    ...
    `  (m) => m.${modePlugin},`,
    `  (err) => {`,
    `    const missingThisPackage =`,
    `      err?.code === 'ERR_MODULE_NOT_FOUND' && String(err.message).includes('${pkg}');`,
    `    if (!missingThisPackage) throw err;`,
    `    return '${mode}';`,
    `  },`,
    `);`,
    ``,
